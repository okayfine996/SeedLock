...
struct EnableBiometricsView: View {
    ...
    @Binding var navigateToEnableBiometrics: Bool
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                // Title
                Text("enable_biometrics.title".localized)
                    .font(.system(size: 28, weight: .bold))
                    .foregroundColor(.appLabel)
                    .padding(.bottom, Theme.spacing12)
                
                // Description
                Text("enable_biometrics.description".localized)
                    .font(.system(size: 17))
                ...
                Button(action: enableBiometrics) {
                    HStack(spacing: Theme.spacing12) {
                        if isAuthenticating {
                            ...
                            Text("enable_biometrics.button".localized)
                ...
                // Subtitle
                Text("enable_biometrics.subtitle".localized)
                    .font(.system(size: 13))
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarLeading) {
                Button(action: {
                    // User cancelled - disable biometrics
                    biometricsRequired = false
                    // Reset navigation state
                    navigateToEnableBiometrics = false
                    dismiss()
                }) {
                    HStack(spacing: 4) {
    ...
    private func enableBiometrics() {
        ...
        BiometricService.shared.authenticate(reason: "enable_biometrics.button".localized) { result in
            DispatchQueue.main.async {
                isAuthenticating = false
                
                switch result {
                case .success:
                    // Keep Biometric Requirement enabled (it's already true from the toggle)
                    // Just log and dismiss
                    
                    // Log event
                    DiagnosticsLogger.shared.logEvent(.success, title: "Biometric Requirement Enabled")
                ...
                case .failure(let error):
                    // Authentication failed - disable biometrics (reset toggle)
                    biometricsRequired = false
...
#Preview {
    NavigationStack {
        EnableBiometricsView(navigateToEnableBiometrics: .constant(true))
    }
}
